...
export class IntegrationAdminGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    const rawAllowed = this.configService.get<string>('INTEGRATION_ADMIN_EMAILS');
    if (!rawAllowed?.trim()) {
      throw new ForbiddenException('Integration admin list is not configured');
    }
    ...
    if (!allowedEmails.includes(email)) {
      throw new ForbiddenException('User is not allowed to manage integrations');
    }
